...
<head>
   ...
   mysql_connect("localhost","root","") or die("error in connection" . mysql_errno());
    ...
    {
    echo "<tr> <td align='center' >$row[Id]</td> <td> $row[FirstName] </td> < <td> $row[Category] </td> <td> $row[A] </td> <td> $row[B] </td> <td> $row[C] </td> <td> $row[D] </td> <td> $row[CorrectOption] </td> <td><a href='edit2.php?id=$row[Id]'>Edit</a></td></tr>";
    }
   echo "</table>";
}
   ?>
